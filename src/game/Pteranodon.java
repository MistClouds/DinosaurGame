...
/**
 * A flying fish-eating dinosaur.
 *
 */
public class Pteranodon extends Actor {
	...
	public Action playTurn(Actions actions, Action lastAction, GameMap map, Display display) {
		...
		if(this.hasSkill(GameSkills.HATCHLING) == true){ // Pteranodon hatchling aging flow and maturation
			hatchlingAge += 1;
			if(hatchlingAge > 20){
				this.displayChar = 'W';
			...
			if(hungerLevel == 0){
				Corpse corpse = new Corpse("Pteranodon Corpse", 'x', true);
				map.locationOf(this).addItem(corpse);
				map.removeActor(this); // Plesiosaur hatchling dies
				return new DoNothingAction();
			} else if(hungerLevel < 10){
				this.removeSkill(GameSkills.HEALTHY);
				System.out.println("Pteranodon at (" + map.locationOf(this).x() + ", " + map.locationOf(this).y() + ") is hungry");
		...
		if(hungerLevel == 0){
			...
			if(randBreed < 0.1){
				Egg egg = new Egg("Pteranodon Egg", 'e', true);
				egg.addSkill(GameSkills.PTERANODON);
